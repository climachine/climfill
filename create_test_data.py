...
def create_gappy_test_data():
    ...
    variables = ['ground temperature','surface layer soil moisture',
                 'precipitation','terrestrial water storage']

    data = xr.DataArray(np.random.rand(len(variables),len(time),len(lat),
                                       len(lon)),
...
def create_constant_test_data():
    ...
    data = xr.DataArray(np.random.rand(len(variables),len(lat),len(lon)),
